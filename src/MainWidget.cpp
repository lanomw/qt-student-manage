...
MainWidget::MainWidget() {
    /**
     * 主界面利用 QHBoxLayout 水平布局划分为左右两个区域
     * 左边区域用 Left_Table_Box 实现
     * 右边区域用 Right_Table_Box 实现
     */
    setWindowIcon(QIcon(":/src/logo.ico"));
...
/**
 * 学生信息面板， Left_Table_Box 中只包含一个 QTableWidget 控件，为了使表格尺寸自适应整个窗口，将表格添加到 QHBoxLayout 中
 */
QGroupBox *MainWidget::createStuMess() {
    ...
    TableWidget->setHorizontalHeaderLabels(QStringList() << "学号" << "姓名" << "年龄" << "语文成绩" << "数学成绩" << "英语成绩");
    flushTable();
...
/**
 * 功能面板
 * 最外层有 QGroupBox 分组框，内部用 QVBoxLayout 将整个区域划分为上下两部分，分别嵌入 QListWidget 和 QGridLayout
 * QGridLayout 内包含添加、删除、保存、推出和1个实现查找的单行输入框
 */
...
void MainWidget::AddStuBox() {
    ...
    // 添加学生信息窗口关闭时，更新表格，同时清空列表中的学生信息
    connect(messBox, &EditStuMessBox::closeBox, this, &MainWidget::flushTable);
    connect(messBox, &EditStuMessBox::closeBox, ListWidget, &QListWidget::clear);
...
void MainWidget::flushTable() {
    ...
    while (!dataStr.atEnd()) {
        ...
        dataStr >> id >> name >> sex >> chinese_score >> math_score >> english_score;
...
void MainWidget::delStuFun() {
    ...
    // 判断用户是否在表格中选中了学生
    if (items.count() > 0) {
        QMessageBox::StandardButton result = QMessageBox::question(this, "删除",
                                                                   "确定要删除学号为【" + items.at(0)->text() + "】的学生吗?");
